...
def load_leaderboard(file_name, leader_names_list, leader_scores_list):
  ...
  for line in leaderboard_file:
    ...
    while (line[index] != ","):
      leader_names_list = leader_names_list + line[index] 
      index = index + 1

    # TODO 2: add the leader name to the list
    leader_names_list.append(leader_names_list)
    
    # TODO 3: read the player score using a similar loop
    index = index + 1
    while (line[index] != "/n"):
      leader_scores_list = leader_scores_list + line[index]
      index = index + 1

    print("leader score is:", leader_scores_list)

    # TODO 4: add the player score to the list
    leader_scores_list.append(int(leader_scores_list))

  leaderboard_file.close()


# update leaderboard by inserting the current player and score to the list at the correct position
def update_leaderboard(file_name, leader_names_list, leader_scores_list, player_name, score):

  leader_index = 0
  # TODO 5: loop through all the scores in the existing leaderboard list
  while (leader_index < len(leader_scores_list)):
    # TODO 6: check if this is the position to insert new score at
    if (score >= leader_scores_list[leader_index]):
      break
    else:
      leader_index = leader_index + 1
  
  # TODO 7: insert the new player and score at the appropriate position
  leader_scores_list.insert(leader_index,score)
  leader_names_list.insert(leader_index,player_name)
  # TODO 8: keep both lists at 5 elements only (top 5 players)
  if (len(leader_names_list) == 6):
      leader_names_list.pop()
  if (len(leader_scores_list) == 6):
      leader_scores_list.pop()
  ...
  while (leader_index < len(leader_names_list)):
      leaderboard_file.write(leader_names_list[leader_index] + "," + str(leader_scores_list[leader_index]) + "\n")
      leader_index = leader_index + 1
...
def draw_leaderboard(leader_names_list, leader_scores_list, high_scorer, turtle_object, score):
  ...
  while leader_index < len(leader_names_list):
    turtle_object.write(str(leader_index + 1) + "\t" + leader_names_list[leader_index] + "\t" + str(leader_scores_list[leader_index]), font=font_setup)
    turtle_object.penup()
  ...
  if (score >= bronze_score and score < silver_score):
      turtle_object.write("... Bronze medal!", font=font_setup)
  elif (score >= silver_score and score < gold_score):
